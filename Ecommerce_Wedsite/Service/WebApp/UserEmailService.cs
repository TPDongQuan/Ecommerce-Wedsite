        ...
        public async Task<ResponseMessageObject<bool>> Email(string checkemail) // Lỗi chưa dùng được gì chưa đăng nhập gmail đc
        {
            try
            {
                ...
                var smtpClient = new SmtpClient("smtp.gmail.com") // Máy chủ SMTP của Gmail
                {
                    ...
                    Credentials = new NetworkCredential("YOUR_EMAIL_HERE", "YOUR_APP_PASSWORD_HERE"), // Thay thế bằng email và mật khẩu của bạn (nên dùng appsettings/user-secrets, không hardcode)
                    EnableSsl = true, // Kích hoạt SSL
                };

                var mailMessage = new MailMessage
                {
                    From = new MailAddress("YOUR_EMAIL_HERE"), // Email người gửi phải giống với email đã nhập ở trên
                    Subject = subject,
                    Body = body,
